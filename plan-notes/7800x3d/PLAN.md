...
**[PLAN-LAPTOP.md](PLAN-LAPTOP.md)**, **[PLAN-GRAVITON.md](PLAN-GRAVITON.md)** and
**[PLAN-PI.md](PLAN-PI.md)**; each runs independently. This file holds the
...
| `plan-x3d.tsv`, `plan-laptop.tsv`, `plan-graviton.tsv`, `plan-pi.tsv` | the four schedules |
...
| `asimd` | ARM. Covered by [PLAN-GRAVITON.md](PLAN-GRAVITON.md) (breadth) and [PLAN-PI.md](PLAN-PI.md) (second µarch) — **not** redundant with `nosimd`: 217 `USE_ARM_V8_SIMD` blocks across 71 files that no x86 plan executes. Note `USE_ARM_V8_SIMD` defines `USE_SSE2` (`platform.h:222`), so ARM shares the 598-set SIMD radix table and uses `worklist_v4_simd.txt` unchanged. |

So the honest answer to "all compatible modes" is **five executable modes on x86**,
`asimd` on ARM (Graviton for breadth, the Pi for a second microarchitecture), and two
that remain build-coverage only. **Mlucas has no SVE path** — ASIMD/NEON 128-bit is the
only ARM SIMD — so Graviton3's SVE units go unused and it runs the same code as the Pi. SDE does not rescue the KNL pair either — SDE
...
| [PLAN-PI](PLAN-PI.md) — confirmation subset on a second ARM µarch; capped at 32M | 7 | — | ~40 h (est.) |
